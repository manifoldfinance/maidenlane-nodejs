...
class ManifoldX extends EventEmitter {
  constructor(settings) {
    ...
    settings = settings || {};
    this._rest_uri = settings.rest_uri || "https://api.maidenlane.io";
  ...
  prepareMarket(symbol, base_asset_id) {
    ...
    if (base_asset_id !== undefined) {
      ...
      market = this._markets.find(
        (m) => m.quote_asset_id === quote_asset_id && m.base_asset_id === base_asset_id
      );
    } else {
  ...
  _handleReport(report) {
    ...
    switch (report.header.type_id) {
      ...
      case Reports.Types.MARKET_READY: {
        const market = this._markets.find(
          (m) =>
            m.quote_asset_id === report.quote_asset_id && m.base_asset_id === report.base_asset_id
        );
        ...
        return erc20.methods
          ...
          .then(() => {
            const encoded = this._dcn.methods
              .user_deposit_to_session(this._user_id, 0, asset_id, deposit_amount)
              .encodeABI();
  ...
  _tradingLimitHash(data) {
    ...
    const data_hash_final = SIG_HASH_HEADER + DCN_HEADER_HASH.substr(2) + data_hash.substr(2);
