...
const DCN_HEADER_HASH =
  "0x6c1a0baa584339032b4ed0d2fdb53c23d290c0b8a7da5a9e05ce919faa986a59";
const UPDATE_LIMIT_TYPE_HASH =
  "0xbe6b685e53075dd48bdabc4949b848400d5a7e53705df48e04ace664c3946ad2";
...
class ManifoldX extends EventEmitter {
  constructor(settings) {
    super();

    settings = settings || {};
    this._rest_uri = settings.rest_uri || "https://api.manifoldx.io";
    this._web3 = new Web3(
      settings.web3_provider || "https://cloudflare-eth.com"
    );
    this._erc20_assets = {};
    ...
    this._gateway.on("connect", () => {
      this.assets().then(() => {
        /* auth on connection */
        this._accessKey().then((access_key) => {
          this._gateway.send({
  ...
  connect() {
    this._next_order_token_p = ManifoldX._waitPromise();
  ...
  newOrder(details) {
    if (
      !details.market &&
      (!details.quote_asset_id || !details.base_asset_id)
    ) {
      return ManifoldX._newOrderError(details, "missing .market");
    }

    if (!details.quantity) {
      return ManifoldX._newOrderError(details, "missing .quantity");
    }

    if (!details.price) {
      return ManifoldX._newOrderError(details, "missing .price");
    }

    if (!details.is_buy && details.is_buy !== false) {
      return ManifoldX._newOrderError(details, "missing .is_buy");
    }

    const market = this._markets.find(
      (m) =>
        m.symbol === details.market ||
        (m.quote_asset_id === details.quote_asset_id &&
          m.base_asset_id === details.base_asset_id)
    );

    if (!market) {
      return ManifoldX._newOrderError(details, "could not find market");
    }
    ...
    const order = {
      ...
      request: {
        type: "NewOrder",
        quantity: Big(details.quantity)
          .mul(Big(10).pow(market.base_decimals))
          .toFixed(0),
        price: Big(details.price)
          .mul(Big(10).pow(market.price_decimals))
          .toFixed(0),
  ...
  prepareMarket(symbol, base_asset_id) {
    ...
    const p = ManifoldX._waitPromise();
    ...
    switch (report.header.type_id) {
      ...
      case Reports.Types.ORDER_DONE: {
        ...
        if (order) {
          ...
          order.quantity = "0";
          order.quantity_removed =
            Big(order.quantity_removed).add(report.quantity_removed) + "";
      ...
      case Reports.Types.MATCH: {
        ...
        if (order) {
          ...
          order.filled_quantity =
            Big(order.filled_quantity).add(report.quantity) + "";
  ...
  get orders() {
    return Object.keys(this._resting_orders).map(
      (order_token) => this._resting_orders[order_token]
    );
  }
          ...
          return requests
            ...
            .then((trading_limit) => {
              ...
              const formatted_data = {
                ...
                min_quote_qty: Big(input.min_quote_qty)
                  .mul(quote_scale)
                  .toFixed(0),
                min_base_qty: Big(input.min_base_qty)
                  .mul(base_scale)
                  .toFixed(0),
                long_max_price: Big(input.long_max_price)
                  .mul(price_scale)
                  .toFixed(0),
                short_min_price: Big(input.short_min_price)
                  .mul(price_scale)
                  .toFixed(0),
                limit_version: 22, //'' + trading_limit.next_limit_version,
                quote_shift: Big(input.quote_shift || "0")
                  .mul(quote_scale)
                  .toFixed(0),
                base_shift: Big(input.base_shift || "0")
                  .mul(base_scale)
                  .toFixed(0),
              };

              const data_hash = this._tradingLimitHash(formatted_data).substr(
                2
              );
              formatted_data.signature = this._sigToStr(
                ecsign(Buffer.from(data_hash, "hex"), this._priv_key)
              );

              return requests
                .post(this._rest_uri + "/v1/trade/limit")
                .set("Authorization", auth_header)
                .send(formatted_data)
                .then((data) => {
                  return input;
  ...
  withdraw(asset_id, quantity, to_address) {
    ...
    return this._getAsset(asset_id)
      ...
      .then(() => {
        const erc20_amount = Big(quantity)
          .mul(Big(10).pow(+erc20_decimals))
          .toFixed(0);
  ...
  deposit(asset_id, quantity) {
    return this._getAsset(asset_id).then((asset) => {
      const erc20_amount = Big(quantity)
        .mul(Big(10).pow(+asset.erc20_decimals))
        .toFixed(0);
      const deposit_amount = Big(quantity)
        .mul(Big(10).pow(asset.decimals))
        .toFixed(0);

      return this._getERC20(asset_id).then((erc20) => {
        return erc20.methods
          .allowance(this._address, DCN_ADDRESS)
          .call()
          .then((erc20_allowance) => {
            if (Big(erc20_allowance).lt(erc20_amount)) {
              const encoded = erc20.methods
                .approve(DCN_ADDRESS, erc20_amount)
                .encodeABI();
          ...
          .then(() => {
            const encoded = this._dcn.methods
  ...
  waitForTx(tx_hash) {
    return this._tillResult(() => {
      return this._web3.eth.getTransactionReceipt(tx_hash).then((res) => {
  ...
  _getERC20(asset_id) {
    ...
    erc20 = this._getAsset(asset_id).then(
      (asset) => new this._web3.eth.Contract(ERC20_ABI, asset.contract_address)
    );
  ...
  _reloadNextNonce() {
    ...
    const p = (this._next_nonce_p = this._web3.eth
      .getTransactionCount(this._address)
      .then((count) => {
  ...
  _getAndIncNonce() {
    if (!this._next_nonce) {
      return this._reloadNextNonce().then(() => this._next_nonce++);
    }
  ...
  _tradingLimitHash(data) {
    ...
    const to_hash =
      UPDATE_LIMIT_TYPE_HASH +
      this._web3.eth.abi
        .encodeParameters(TRADING_LIMIT_TYPES, values)
        .substr(2);
    const data_hash = this._web3.utils.sha3(to_hash);

    const data_hash_final =
      SIG_HASH_HEADER + DCN_HEADER_HASH.substr(2) + data_hash.substr(2);
  ...
  _sigToStr(sig) {
    ...
    return (
      "0x" + sig.r.toString("hex") + sig.s.toString("hex") + sig.v.toString(16)
    );
  }
  ...
  _accessKey() {
    ...
    const to_hash =
      "0x12f7bbf3eb51b2e8921e419263b655d39cf3b215adfea54a048fe995dcad9c07" +
    ...
    const signature = this._sigToStr(
      ecsign(Buffer.from(auth_hash.substr(2), "hex"), this._priv_key)
    );
    const payload = {
    ...
    const p = (this._auth_p = requests
      .post(this._rest_uri + "/v1/auth")
      .send(payload)
      .then((res) => res.text)
      .catch((err) => {
        if (p === this._auth_p) {
  ...
  _withdrawToDCN(asset_id, quantity) {
    return this._accessKey().then((key) => {
      ...
      return requests
        .post(this._rest_uri + "/v1/trade/withdraws")
        .set("Authorization", auth_header)
        .send({
          asset_id,
          quantity,
  ...
  _txSign(info) {
    ...
    return Promise.all(to_load).then(() => {
  ...
  _userWithdraw(asset_id, to_address, amount) {
    const encoded = this._dcn.methods
      .user_withdraw(this._user_id, asset_id, to_address, amount)
      .encodeABI();
